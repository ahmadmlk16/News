...
var listOfSearchNews = [NewsObject]()
...
public func obtainSearchNewsFromApi(endpoint: String, dateFrom : String, dateTo: String, sortBy: String, searchQuery : String) {

    
    var apiUrl = ""
    if endpoint == "topheadlines" {
        apiUrl = "https://newsapi.org/v2/top-headlines?q=\(searchQuery)&language=en&from=\(dateFrom)&to=\(dateTo)&sortBy=\(sortBy)"
    } else {
        apiUrl = "https://newsapi.org/v2/everything?q=\(searchQuery)&language=en&from=\(dateFrom)&to=\(dateTo)&sortBy=\(sortBy)"
    }
            ...
            for _ in newsJsonArray{
                ...
                if let jObject = newsJsonArray[i] as?[String: Any]{
                    ...
                    //append the new News item into the list
                                        listOfSearchNews.append(NewsObject(id: nId, sourceName : nSourceName, author: nAuthor, title: nTitle, description: nDescription, url: nUrl, urlToImage: nUrlToImage, publishedAt: nPublishedAt, content: nContent))
